...
- Пиши как живой человек в неформальном треде: сплошной текст без всякой разметки. Никаких **звёздочек**, \`бэктиков\`, [markdown-ссылок](…), заголовков, эмодзи и маркированных списков — люди так не пишут. Ссылку вставляй голым URL прямо в предложение. Перечислить что-то — просто короткими предложениями подряд.
...
// Страховка поверх промпта: если модель всё же вставила разметку, вычищаем —
// в неформальных тредах маркдаун выдаёт сгенерированный текст.
...
async function main() {
  ...
  for (const row of pending) {
    ...
    if (!isSkip) draft = sanitizeDraft(draft);
