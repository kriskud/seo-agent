...
export function createDailyBudget({ env = process.env,
  file = join(ROOT, 'data/seeding/yandex-budget.json'), clock = () => new Date() } = {}) {
  const value = env.YANDEX_SEARCH_DAILY_LIMIT ?? '50';
  if (!/^\d+$/.test(value) || !Number.isSafeInteger(Number(value))) {
    throw new Error('YANDEX_SEARCH_DAILY_LIMIT must be a non-negative integer');
  }
  ...
  return {
    ...
    reserve() {
      ...
      try {
        ...
        if (state.used >= limit) {
          const e = new Error(`Daily Yandex request limit reached (${state.used}/${limit}, UTC). No HTTP request sent.`);
        ...
        const e = cause.code === 'SEARCH_BUDGET_EXHAUSTED' ? cause
          : new Error('Cannot safely reserve Yandex daily budget; no HTTP request sent. Check budget file and lock.');
        e.fatal = true;
        throw e;
      } finally { release?.(); }
    },
