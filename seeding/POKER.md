...
Вьюер — единый пульт конвейера: разметка статусов, кнопка «Черновик» у строк,
для которых драфтер уже сгенерировал ответ (текст + «Скопировать» + «открыть
тред»), фильтр «только с черновиком». Черновики попадают в реестр из
draft.mjs через seeding/set-draft.mjs (ssh, под локом). После новой разметки
relevant — снова запустить драфтер локально, страницу обновить.
